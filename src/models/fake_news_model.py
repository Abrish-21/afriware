...
OUTPUT_DIR    = ROOT / "models" / "fake_news"
...
DEFAULT_CONFIG = {
    ...
    "max_length":     256,      # news articles are longer than tweets
...
def compute_metrics(eval_pred):
    ...
    f1  = _f1_metric.compute(predictions=preds, references=labels, average="binary")
    acc = _acc_metric.compute(predictions=preds, references=labels)

    return {"f1_binary": f1["f1"], "accuracy": acc["accuracy"]}
...
def build_trainer(config: dict):
    ...
    train_ds  = FakeNewsDataset(train_df, tokenizer, config["max_length"])
    val_ds    = FakeNewsDataset(val_df,   tokenizer, config["max_length"])
...
def train(config: dict = None):
    ...
    print(f"\n{'='*55}")
    print("AfriAware — Fake News Model Training")
    print(f"{'='*55}")
    ...
    test_ds = FakeNewsDataset(test_df, tokenizer, config["max_length"])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="AfriAware fake news model")
